...
/////////Get message on  Liveobject data //////////////////////////
...
{
  {"Temp", "Hygro", "PsAt", "Lum", "Bruit", "Alt", "tempCaf", "Message", "Heure", "Date", "IP","RSSI","Box","SourceRss","Status"},
...
void otaConfiguration()
{
  ...
  ArduinoOTA.setPassword((const char *)"1");
 ...
 if (currentMillis - previousMillis >= interval)
  {
    previousMillis = currentMillis;
      ...
      temp = dht.readTemperature();//Convertion float to int
      Serial.print("Temperature:");
      Serial.print(temp);
      Serial.print("*C");
    
      Serial.print(" | humidity:");
      humi = dht.readHumidity() + offsetHumid;
      Serial.print(humi);
      Serial.print("%");
      ...
      if (!bmp.begin())
      {
        ...
        DispStatus =  "Capteur cafetiere non fonctionnel" ;
        bmplock = 1;
      }
      if ( bmplock  == 0)
      {
...
void LiveObjectDataParser()
{
    ...
    memset(charTempCaf,'\0',1);
     ...
     { 
       if(jsonArayOK[i]=='M'&& jsonArayOK[i+1]=='s'&& jsonArayOK[i+2]=='g'&& jsonArayOK[i+3]=='"'&& jsonArayOK[i+4]==':')
       {
          Serial.println("Temp caf match");
          int j = 0;
          
          while(jsonArayOK[i+j+5] !='}')
          {
            charTempCaf[j]=jsonArayOK[i+5+j];
            j=j+1;
          }
